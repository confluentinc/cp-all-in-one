#####################################################
# The following values are provided by Confluent's
# release automation process.  The values in these
# names will be adopted in scripts and expressions
# over time to construct other
# values required by this repository
#####################################################
export CONFLUENT=7.7.0
export CONFLUENT_DOCKER_TAG=7.7.0
export CONFLUENT_SHORT=7.7
export CONFLUENT_PREVIOUS="7.7.0"
export CONFLUENT_RELEASE_TAG_OR_BRANCH=7.7.1-post
export CONFLUENT_MAJOR=7
export CONFLUENT_MINOR=7
export CONFLUENT_PATCH=0
#####################################################

export CP_VERSION_FULL="$CONFLUENT_MAJOR.$CONFLUENT_MINOR.$CONFLUENT_PATCH"
